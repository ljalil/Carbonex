...
class DuanSun2006:
    ...
    def co2_fugacity_coefficient(self, P: float, T: float) -> float:
        """
    ...
    def calculate_CO2_solubility(
        ...
        try:
            ...
            prod = self.calculate_CO2_vap_mol_frac(P, T) * P_bar * self.co2_fugacity_coefficient(P, T)
            if prod <= 0:
                print(f"Negative product: {prod} (T={T} K, P={P/10} MPa, molalities={molalities})")
                print(f"CO2 fugacity: {self.co2_fugacity_coefficient(P, T)}")
    ...
    def calculate_varying_pressure(
        self,
        P_start: float,
        P_end: float,
        P_step: float,
        ...
        """
        Calculate CO2 solubility over a range of pressures at constant temperature and ion molalities.

        Parameters:
            P_start: Starting pressure in MPa
            P_end: Ending pressure in MPa
            P_step: Pressure increment in MPa
            T: Temperature in Kelvin
            molalities: Dictionary of ion molalities (keys like 'Na+', 'Cl-')
            model: 'DuanSun' or 'Guo'

        Returns:
            DataFrame with columns 'Pressure (MPa)' and 'CO2_solubility (mol/kg)'.
        """
        print('from inside DuanSUn2006', flush=True)
        if molalities is None:
            molalities = {}
        # Prepare pressure array
        pressures = np.arange(P_start, P_end + P_step, P_step).tolist()
        solubilities = []
        for P in pressures:
            sol = self.calculate_CO2_solubility(P, T, molalities, model)
            solubilities.append(sol)
        
        #print('duan sun pressures' ,pressures, file=sys.stdout, flush=True)
        #print('duan sun solubilities' ,solubilities, file=sys.stdout, flush=True)
