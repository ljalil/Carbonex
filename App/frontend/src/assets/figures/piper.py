...
def plot(df, 
    ...
    ax = fig.add_subplot(111, aspect='equal', frameon=False, 
                         xticks=[], yticks=[])
    ax.plot(ltriangle_x, ltriangle_y, '-k', lw=1)
    ...
    for i, x in enumerate(np.linspace(0, 1, int(1/interval+1))):
        ...
        if i in [1, 2, 3, 4]:
            ax.text((1-x)/2.0+x + 0.026, (1-x)/2.0*np.tan(np.pi/3) + 0.015, 
                    ticklabels[i], ha='center', va='center', rotation=-60, fontsize=fontsize)
        ax.plot([x/2, 1-x/2], 
        ...
        if i in [1, 2, 3, 4]:
            ax.text(x+1+2*offset, 0-0.03, 
                    ticklabels[i], ha='center', va='center' , fontsize=fontsize)
        ...
        if i in [1, 2, 3, 4]:
            ax.text((1-x)/2.0+x+1+2*offset  + 0.026, (1-x)/2.0*np.tan(np.pi/3) + 0.015, 
                    ticklabels[-i-1], ha='center', va='center', rotation=-60 , fontsize=fontsize)
        ...
        if i in [1, 2, 3, 4]: 
            ax.text(0.5+offset+0.5/(1/interval)*x/interval  - 0.026, h+offset*np.tan(np.pi/3)+0.5/(1/interval)*x/interval*np.tan(np.pi/3) + 0.015, ticklabels[i], 
                    ha='center', va='center', rotation=60, fontsize=fontsize)
    ...
    if (df['Color'].dtype is np.dtype('float')) or (df['Color'].dtype is np.dtype('int64')):
        ...
        cb.ax.set_ylabel('$TDS$' + ' ' + '$(mg/L)$', rotation=90, labelpad=-75, fontsize=8)
    
    #plt.legend(bbox_to_anchor=(0.15, 0.875), markerscale=1, fontsize=fontsize,
    #           frameon=False, 
...
if __name__ == '__main__':
    ...
    plot(df, unit='mg/L', figname='piper', figformat='jpg')